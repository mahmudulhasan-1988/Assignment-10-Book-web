...
export default function MyReviews() {
  const { reviews: rawReviews, loading, updateReview, deleteReview } = useReviews();
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--rr-hairline)] bg-[var(--rr-surface)] shadow-lg">
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-[var(--rr-ink-dim)]">
          <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-[var(--rr-bg)]">
        ...
        <div className="divide-y divide-[var(--rr-hairline)]">
          {reviews.map((review) => {
            ...
            return (
              <div
                key={review._id}
                className="group transition-colors hover:bg-[var(--rr-bg)]/50"
              >
                      ...
                      <div className="mt-3 space-y-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--rr-gold)] px-3 py-1.5 text-xs font-medium text-white hover:bg-[var(--rr-gold-bright)] transition-colors"
                          >
                            Save Changes
                          </button>
                          <button
                            onClick={cancelEdit}
                            className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--rr-hairline)] px-3 py-1.5 text-xs font-medium text-[var(--rr-ink-dim)] hover:bg-[var(--rr-surface-2)] transition-colors"
                          >
                            Cancel
                          </button>
                        </div>
                      ...
                      {isConfirming ? (
                        <div className="flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 px-3 py-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-[var(--rr-hairline)] px-3 py-1.5 text-xs font-medium text-[var(--rr-ink-dim)] hover:bg-[var(--rr-surface-2)] transition-colors"
                          >
                            Cancel
                          </button>
                        </div>
